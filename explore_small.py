...
documentList = documentList[50000:100000]
SentencesList = SentencesList[50000:100000]
...
def getSortedRowIndex(row, n,this_list):
	sortedList = []
	
	for i in range(0,n):
		maxValue = -999999;
		maxIndex = -1
		for col in range(0,len(this_list[row])):
			if(this_list[row][col] > maxValue):
				exist = False
				for j in range(0,len(sortedList)):
					if (sortedList[j] == col):
...
def ASUM(dictionary,sentiWordsList,SentencesList):
	...
	with open('phi_523.pkl', 'wb') as f:
		pickle.dump(Phi,f)

	with open('theta_523.pkl', 'wb') as f:
		pickle.dump(Theta,f)
...
def my_print():
	...
	num_documents = 50000
	# num_Prob_Words = 100
	with open('phi_523.pkl', 'rb') as f:
		Phi = pickle.load(f)

	with open('theta_523.pkl', 'rb') as f:
		Theta = pickle.load(f)

	with open('pi_523.pkl', 'rb') as f:
		Pi = pickle.load(f)
	...
	num_top_topics = 2
	sentiTopicCount = np.zeros((n_topics,n_sent),dtype=np.int32)
	topicIndices = np.zeros((num_documents,n_sent,num_top_topics),dtype=np.int32)
	for d in range(0,num_documents):
		for s in range(0,n_sent):
			sortedIndexList = getSortedRowIndex(d,num_top_topics,Theta[s])
			for k in range(0,len(sortedIndexList)):
				topicIndices[d][s][k] = sortedIndexList[k]
	for k in range(0,num_top_topics):			
		for d in range(0,num_documents):
			for s in range(0,n_sent):
				index = topicIndices[d][s][k]
				sentiTopicCount[index][s] += 1
	for t in range(0,n_topics):
		for s in range(0,n_sent):
			print("S"+str(s)+"-T"+str(t)+",",end=" ")
	print("\n")
